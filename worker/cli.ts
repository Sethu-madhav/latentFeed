...
async function main() {
  ...
  for (const r of summary.results) {
    if (!r.ok) {
      ...
      console.log(
        `  ✓ ${r.slug.padEnd(26)} seen ${String(r.seen).padStart(3)}  new ${String(r.inserted).padStart(3)}  dup ${String(r.duplicates).padStart(3)}  ${r.durationMs}ms`,
      );
    }
  }

  console.log(
    `\npolled ${summary.polled} · inserted ${summary.inserted} · duplicates ${summary.duplicates} · failed ${summary.failed}`,
  );
